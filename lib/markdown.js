...
Markdown.dialects.Gruber = {
  block: {
    ...
    cdata: function cdata( block, next ) {
      var ret = [],
          reStart = /^\s*\<\[\!CDATA\[(.*)/,
          reEnd = /(.*)\]\]\>\s*/,
          lines;
      var m = block.match(reStart); // <[!CDATA[ <span1> \n <span2> \n <span3> ]]>
      if ( !m ) return undefined;

      var raw = m[1];
      var m2 = raw.match(reEnd);
      if ( m2 ) {
        ret.push(m2[1]);
      }
      else {
        ret.push(raw);
        next.unshift(block.valueOf().substring(m[0].length));
        // find ]]>
        while (next.length) {
          var b = next[0].valueOf(); // next block
          next.shift();
          var lines = b.split('\n'); // next block lines
          var exitWhile = false;
          for (var i = 0; i < lines.length; i ++ ) {
            var line = lines[i];
            var m3 = line.match(reEnd);
            if (m3) {
              // matched:
              ret.push(m3[1]);
              if ((i+1) < lines.length) {
                next.unshift(lines.slice(i+1).join('\n'));
              }
              exitWhile = true;
              break;
            }
...
function convert_tree_to_html( tree, references, options ) {
  ...
  switch ( jsonml[ 0 ] ) {
    ...
    case "cdata":
      if (! options.cdata) {
        jsonml[1] = "<!-- CDATA is disabled by default. -->"
      }
      break;
    case "inlinecode":
      jsonml[ 0 ] = "code";
